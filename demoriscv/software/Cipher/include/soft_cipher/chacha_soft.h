...
static void chacha20_init_state(uint32_t s[16], uint8_t key[32], uint32_t counter, uint8_t nonce[12]) {
    ...
    s[1] = 0x3120646e;      // for "expand 16-byte k"
    s[2] = 0x79622d36;
...
static void ChaCha20XOR(uint8_t key[32], uint32_t counter, uint8_t nonce[12], uint8_t *in, uint8_t *out, int inlen) {
    ...
    for (i = 0; i < inlen; i += 64) {
        chacha20_block(s, block, 8); // round change
        s[12]++;
...
static void chacha_soft_main(void) {
...
//   printf("#============================================================================\n");
//   printf("ChaCha (8 rounds) stream cipher, software implementation,\n");
   // printf("ONE-stream-block test case, minimal setup, no printing on terminal,\n");
   // printf("clock cycles NOT counted for preparing key, nonce, and message, just cipher!\n");
   ...
   // ================================
   ChaCha20XOR(key, 0, nonce, input, encrypt, 64);
   // ================================


   count_cycles += read_csr(mcycle);
   kprintf("# ChaCha20 - Software Implementation  ===============================\r\n");
