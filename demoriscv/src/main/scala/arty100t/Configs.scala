...
import demoriscv.fpga.arty100t.{WithArty100TDDRTL, WithArty100TJTAG, WithArty100TPMODUARTs}
...
class WithSimpleBootROM extends Config((site, here, up) => {
  case BootROMLocated(x) => up(BootROMLocated(x), site).map{ p =>
    ...
    val clean = s"make -C demoriscv/src/main/resources/bootROM/basic clean"
    require (clean.! == 0, "Failed to clean")
    // Build the bootrom
    val make = s"make -C demoriscv/src/main/resources/bootROM/basic XLEN=64 PBUS_CLK=${freqMHz}"
    require (make.! == 0, "Failed to build bootrom")
    p.copy(hang = 0x10000, contentFileName = s"./demoriscv/src/main/resources/bootROM/basic/build/sdboot.bin")
  }
})
